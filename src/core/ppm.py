...
class ProcessPreferenceModel(nn.Module):
    ...
    def evaluate_step(self, step: str, embedder: Any) -> float:
        ...
        with torch.no_grad():
            ...
            embedding = embedder.embed_text(step)
    ...
    def train_step(self, 
        ...
        preferred_embeddings = torch.FloatTensor([
            embedder.embed_text(step) for step in preferred_steps
        ])
        non_preferred_embeddings = torch.FloatTensor([
            embedder.embed_text(step) for step in non_preferred_steps
        ])
    ...
    def _validate(self, 
        ...
        with torch.no_grad():
            for batch in self._create_batches(validation_data, self.model.config.batch_size):
                preferred_embeddings = torch.FloatTensor([
                    embedder.embed_text(step) for step in batch['preferred']
                ])
                non_preferred_embeddings = torch.FloatTensor([
                    embedder.embed_text(step) for step in batch['non_preferred']
                ])
